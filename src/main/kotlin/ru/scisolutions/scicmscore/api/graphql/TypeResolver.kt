...
class TypeResolver {
    fun objectType(attrName: String, attribute: Attribute): GraphQLType<*> =
        when (val attrType = AttrType.valueOf(attribute.type)) {
            ...
            AttrType.relation -> {
                ...
                val attrRelType = AttrRelType.valueOf(attribute.relType)
            ...
            else -> throw IllegalArgumentException("Attribute [$attrName] has unsupported type (${attrType})")
        }
    ...
    fun filterInputType(attrName: String, attribute: Attribute): GraphQLType<*> =
        when (val attrType = AttrType.valueOf(attribute.type)) {
            ...
            AttrType.int -> TypeNames.INT_FILTER_INPUT
            AttrType.float,
            AttrType.decimal -> TypeNames.FLOAT_FILTER_INPUT
    ...
    fun inputType(attrName: String, attribute: Attribute): GraphQLType<*> =
        when (val attrType = AttrType.valueOf(attribute.type)) {
            ...
            AttrType.int -> TypeNames.INT
            AttrType.float,
            AttrType.decimal -> TypeNames.FLOAT
            ...
            AttrType.array,
            AttrType.json,
            AttrType.media -> TypeNames.STRING
            AttrType.relation -> {
                requireNotNull(attribute.target) { "Attribute [$attrName] has a relation type, but target is null" }
                requireNotNull(attribute.relType) { "Attribute [$attrName] has a relation type, but relType is null" }
