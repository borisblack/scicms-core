...
object SQL {
    fun toSqlValue(value: Any?) = when (value) {
    ...
    fun toSqlValue(value: Any?, type: FieldType) = when (type) {
        FieldType.uuid,
        FieldType.string,
        FieldType.enum,
        FieldType.sequence,
    ...
    fun toSqlValueWithParsing(value: Any?) = if (value is String) parseStringValue(value) else toSqlValue(value)

    @Deprecated("Incorrect parsing in case of numerical strings with non-numerical meaning")
    private fun parseStringValue(value: String): Any = if (Numeric.isInt(value)) {
        value.toInt()
