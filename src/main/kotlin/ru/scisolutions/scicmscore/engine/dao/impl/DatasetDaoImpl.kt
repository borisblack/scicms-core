...
class DatasetDaoImpl(private val jdbcTemplateMap: JdbcTemplateMap) : DatasetDao {
    override fun findAll(dataset: Dataset, start: String?, end: String?): List<Map<String, Any?>> {
        val paramSource = DatasetSqlParameterSource()
        val query = datasetQueryBuilder.buildFindAllQuery(dataset, start, end, paramSource)
