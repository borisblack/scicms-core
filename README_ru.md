...
- MAIN_DB_URL - стандартная строка соединения JDBC (в настоящий момент поддерживаются СУБД PostgreSQL, Oracle и SQLite);
...
Если используется встроенная БД SQLite, то при первом запуске создается файл по пути, указанному в строке подключения.
